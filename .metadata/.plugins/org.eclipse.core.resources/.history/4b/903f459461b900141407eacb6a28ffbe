package CardGame;
/**
 * nothing here yet. just need to make sure it's all working
 * @author Bernard Serbinowski
 * @author Seth Zebrack
 */
public class Card {
	
	private Object cost;//this will probably be changed when we figure out what we want to call resources and shit
	private boolean resourceConvertable;//can this be converted to a resource
	private String name;//the name of the card
	private enum Type{
		ARMOR, MAGIC, OTHER;//place holder but like will probably be changed later or something
	}
	
	
	public Card(){
		
	}
	
}
